...
static int socket_setup(void)
{
	...
	zassert_equal(0, tls_credential_add(TLS_TAG_INFUSE_COAP, TLS_CREDENTIAL_PSK_ID,
					    test_identity, sizeof(test_identity)));
	zassert_equal(0, tls_credential_add(TLS_TAG_INFUSE_COAP, TLS_CREDENTIAL_PSK, test_psk,
					    sizeof(test_psk)));

	/* IPv4 lookup */
	zassert_equal(0, infuse_sync_dns(coap_test_server, 5684, AF_INET, SOCK_STREAM, &address,
					 &address_len));
