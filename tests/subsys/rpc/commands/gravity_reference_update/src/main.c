...
ZTEST(rpc_command_gravity_reference_update, test_variance_bad)
{
	...
	k_sleep(K_MSEC(100));
	zbus_chan_pub(CHAN, &base, K_FOREVER);
	rsp = expect_gravity_reference_update_response(105,
						       INFUSE_RPC_ERROR_DATA_VARIANCE_TOO_HIGH);
	...
	k_sleep(K_MSEC(100));
	zbus_chan_pub(CHAN, &base, K_FOREVER);
	rsp = expect_gravity_reference_update_response(106,
						       INFUSE_RPC_ERROR_DATA_VARIANCE_TOO_HIGH);
	validate_data((void *)rsp->data, false);
	net_buf_unref(rsp);
	...
	k_sleep(K_MSEC(100));
	zbus_chan_pub(CHAN, &base, K_FOREVER);
	rsp = expect_gravity_reference_update_response(107,
						       INFUSE_RPC_ERROR_DATA_VARIANCE_TOO_HIGH);
	validate_data((void *)rsp->data, false);
	net_buf_unref(rsp);
}
