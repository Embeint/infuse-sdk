...
#ifndef INFUSE_SDK_INCLUDE_INFUSE_MATH_MEAN_H_
#define INFUSE_SDK_INCLUDE_INFUSE_MATH_MEAN_H_
...
/**
 * @brief Initializer for @ref mean_state.
 */
#define INFUSE_MEAN_INIT() {0}

/**
 * @brief Resets the mean calculation state.
 *
 * @param state Pointer to the `mean_state` structure to reset.
 */
static inline void mean_reset(struct mean_state *state)
{
	state->n = 0;
	state->m = 0;
}
...
#endif /* INFUSE_SDK_INCLUDE_INFUSE_MATH_MEAN_H_ */
