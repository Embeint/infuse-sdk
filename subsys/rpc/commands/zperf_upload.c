			    ...
			    uint32_t len)
{
	 ...
	 * earliest data. Duplicating data is no issue, this is only for throughput purposes.
	 */
	block %= state.current_block;
	block = MAX(state.earliest_block, block);
...
static int zperf_upload_data_loader(void *user_ctx, uint64_t offset, uint8_t *data, uint32_t len)
{
	...
	if (encrypt) {
		 ...
		 * in buffer overflows. Truncated data being sent is no issue, as this is only for
		 * throughput purposes.
		 */
		if (len < 24) {
			len = 24;
