...
struct net_buf *rpc_command_data_sender(struct net_buf *request)
{
	...
	while (remaining > 0) {
		...
		data_buf = epacket_alloc_tx_for_interface(rx_meta.interface, K_FOREVER);
		if (net_buf_tailroom(data_buf) == 0) {
			/* Backend connection has been lost */
			net_buf_unref(data_buf);
			break;
		}
		epacket_set_tx_metadata_core(data_buf, rx_meta.auth, rx_meta.key_identifier, 0x00,
					     INFUSE_RPC_DATA, EPACKET_ADDR_ALL);
		...
		epacket_queue(rx_meta.interface, data_buf);
		remaining -= tail;
		tx_offset += tail;
	}

	/* Allocate and return response */
	return rpc_response_simple_if(rx_meta.interface, 0, &rsp, sizeof(rsp));
}
