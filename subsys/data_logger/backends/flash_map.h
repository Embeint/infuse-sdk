...
#define DATA_LOGGER_BACKEND_CONFIG_FLASH_MAP(node, data_ptr)                                                           \
	...
	.backend_config = {                                                                                            \
		...
		.logical_blocks = (DT_REG_SIZE(DT_PROP(node, partition)) / BLOCK_SIZE) * MAX_WRAPS,                    \
		.physical_blocks = (DT_REG_SIZE(DT_PROP(node, partition)) / BLOCK_SIZE),                               \
